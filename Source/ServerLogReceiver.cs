...
namespace DiscordTools
{
    internal static class ServerLogReceiver
    {
        ...
        private static readonly TimeSpan TransferExpiry = TimeSpan.FromMinutes(10);
        ...
        public static void OnMetadata(long sender, ZPackage pkg)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(transfer.RequestId))
                {
                ...
                if (Transfers.TryGetValue(transfer.RequestId, out var oldTransfer))
                {
                    TryDelete(oldTransfer.TempPath);
                }
            ...
            {
                DiscordToolsPlugin.Log.LogWarning("Ignored malformed client log metadata RPC from " + sender + ": " + ex.Message);
            }
        }
        ...
        public static void OnChunk(long sender, ZPackage pkg)
        {
            ...
            {
                DiscordToolsPlugin.Log.LogWarning("Ignored malformed client log chunk RPC from " + sender + ": " + ex.Message);
            }
        }
        ...
        private static void SendResult(long target, string requestId, bool success, string message)
        {
            ...
            {
                DiscordToolsPlugin.Log.LogWarning("Could not send client log result because ZRoutedRpc is not ready.");
                return;
            }
            ...
            {
                DiscordToolsPlugin.Log.LogWarning("Could not send client log result RPC to " + target + ": " + ex.Message);
            }
        }
        ...
        private static void CleanupExpiredTransfers()
        {
            ...
            var cutoff = DateTime.UtcNow - TransferExpiry;
            var expired = new List<string>();
            foreach (var entry in Transfers)
            {
                if (entry.Value.ReceivedAtUtc < cutoff)
                {
            ...
            {
                var transfer = Transfers[requestId];
                TryDelete(transfer.TempPath);
                Transfers.Remove(requestId);
                DiscordToolsPlugin.Log.LogWarning("Expired incomplete client log transfer request=" + requestId + ".");
            }
        }
